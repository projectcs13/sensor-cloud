...
%% @doc
%% Function: create_streams_table/0
%% Purpose: Create a table 'streams' in the database for streams.
%% Returns: {atomic, ok} | {aborted, Reason}
%%
%% Side effects: Create 'streams' table in the database following the
%%               specification of the record #streams.
%% @end
-spec create_streams_table() -> {atomic, ok} | {aborted, term()}.
create_streams_table() ->
  case mnesia:create_table(streams,
						 [{attributes, record_info(fields, streams)},
						{disc_copies, ?NODES},
						{type, ordered_set}])
	of
	{atomic, ok} -> {atomic, ok};
...
%%
%% Side effects: Inserts a row in the table 'streams' in the database
%%               with the specified data.
%%               Id of the stream is set to the highest id in the table + 1.
%% @end
-spec add_stream(record) -> {aborted, term()} | ok | {error, term()}.
add_stream(Streams_record) when is_record(Streams_record, streams) ->
	Trans = fun() ->
					case mnesia:last(streams) of
						'$end_of_table' -> Id = 1;
						I -> Id = I + 1
					end,
					mnesia:write(Streams_record#streams{id=Id})
			end,
	mnesia:wait_for_tables([streams], 5000),
...
%% Returns: {aborted, Reason} | {error, Reason} | [Record] (#streams)
%%
%% @end
-spec get_all_streams() -> {aborted, term()} | {error, term()} | record.
get_all_streams() ->
	Trans = fun() ->
					case mnesia:match_object(#streams{_='_'}) of
						{aborted, Reason} -> {aborted, Reason};
						[] -> {error, "No Such ID"};
						S -> S
					end
...
get_stream_by_id(Id) when is_integer(Id) ->
	Trans = fun() ->
					case mnesia:match_object(#streams{id=Id, _='_'}) of
						{aborted, Reason} -> {aborted, Reason};
						[] -> {error, "No Such ID"};
						[S|_] -> S
					end
...
update_stream(Stream) when is_record(Stream, streams) ->
	case get_stream_by_id(Stream#streams.id) of
		{aborted, Reason} -> {aborted, Reason};
		...
		Record ->
			Updated_record =
				Record#streams{
							   type =
								   case Stream#streams.type of
									   undefined -> Record#streams.type;
									   Type -> Type
								   end,
							   latitude =
								   case Stream#streams.latitude of
									   undefined ->
										   Record#streams.latitude;
									   Lat -> Lat
								   end,
							   longitude =
								   case Stream#streams.longitude of
									   undefined ->
										   Record#streams.longitude;
									   Long -> Long
								   end,
							   description =
								   case Stream#streams.description of
									   undefined ->
										   Record#streams.description;
									   Desc -> Desc
								   end,
							   public_access =
								   case Stream#streams.public_access of
									   undefined ->
										   Record#streams.public_access;
									   P_A -> P_A
								   end,
							   public_search =
								   case Stream#streams.public_search of
									   undefined ->
										   Record#streams.public_search;
									   P_S -> P_S
								   end,
							   frozen =
								   case Stream#streams.frozen of
									   undefined -> Record#streams.frozen;
									   Frozen -> Frozen
								   end,
							   history_size =
								   case Stream#streams.history_size of
									   undefined ->
										   Record#streams.history_size;
									   H_S -> H_S
								   end,
							   last_updated =
								   case Stream#streams.last_updated of
									   undefined ->
										   Record#streams.last_updated;
									   L_U -> L_U
								   end,
							   secret_key =
								   case Stream#streams.secret_key of
									   undefined -> Record#streams.secret_key;
									   S_K -> S_K
								   end,
							   owner_id =
								   case Stream#streams.owner_id of
									   undefined -> Record#streams.owner_id;
									   Owner_ID -> Owner_ID
								   end,
							   resource_id =
								   case Stream#streams.resource_id of
									   undefined -> Record#streams.resource_id;
									   Res_ID -> Res_ID
								   end,
							   version =
								   case Stream#streams.version of
									   undefined -> Record#streams.version;
									   Version -> Version
								   end
							  },
			Trans = fun() ->
...
%%
%% Side effect: Deletes a row from the database in the table 'streams'
...
delete_stream_with_id(Id) when is_integer(Id) ->
	Trans = fun() ->
					case mnesia:delete({streams, Id}) of
						{aborted, Reason} -> {aborted, Reason};
						ok -> ok
					end
			end,
...
print_stream(#streams{id=Id, type=Type, latitude=Lat, longitude=Long,
...
%% Purpose: Clear all entries in the 'streams' table.
%% Returns: {atomic, ok} | {aborted, Reason}
%%
%% Side effect: Clears all entries in the 'streams' database table.
%% @end
-spec clear_streams_table() -> {atomic,ok} | {aborted, term()}.
clear_streams_table() ->
	mnesia:wait_for_tables([streams], 5000),
	mnesia:clear_table(streams).
